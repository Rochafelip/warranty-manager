      ...
      <div v-if="invoice" class="invoice-details">
        <div class="info-section">
          ...
          <p>
            ...
            <span v-if="invoice.pdf_url">
              <a :href="invoice.pdf_url" target="_blank" rel="noopener noreferrer" class="link">Abrir PDF</a>
            </span>
            <span v-else class="error-message">PDF não disponível.</span>
          </p>
        </div>
...
export default {
  ...
  async mounted() {
    this.isLoading = true;
    try {
      const response = await api.get(`/invoices/${this.$route.params.id}`);
